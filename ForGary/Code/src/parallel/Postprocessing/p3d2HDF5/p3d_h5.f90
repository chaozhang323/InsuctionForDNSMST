...
PROGRAM p3d_h5
  ...
  integer ::   i, j, k, kk, k2, kp, n, nn,nnn, imax, jmax, kmax, ires, icurrent, jres, jcurrent
  ...
  character(400) :: fname
  character(8) :: fnum
  character(4) :: fnum4_1, fnum4_2, fnum4_3, fnum4_4
  ...
  if(iConvert_flow.eq.1) then ! Convert flow file
    ...
    if(iFormat_flow.eq.3) then !kioplane
      ...
      do nn=1, num_file !reading each file given
        ...
        write(unit=fnum,fmt='(I08.8)') (file_be + (nn-1)*file_skip)
          ...
          !defining plane
          do kk=1, kmax, kioplane !kk is the start
            k2 = min(kk+kioplane-1,kmax) !k2 is the end
            kp=k2-kk+1 !kp is the size
            
            fsol_new%dimsf = (/kp,decomp%xsz(2),jmax/)
          
            if((nn.eq.1).and.(kk.eq.1)) allocate(buffer(kp,decomp%xsz(2),jmax)) 
            
            !generating new file name
            write(unit=fnum4_1,fmt='(I04.4)') kk !kiostart
            write(unit=fnum4_2,fmt='(I04.4)') k2 !kioend
            fsol_new%fname = 'flowdata_'//fnum//'_kplane'//fnum4_1//'_'//fnum4_2//'.h5'
            if(myid.eq.0) print *, 'Making file ... ', trim(fsol_new%fname)            
            
            
            do n=1, fsol%dnum !reading single varibles
            

               if(myid.eq.0) print *, 'Reading variable: ', trim(fsol%dname(n))
               
               if ((kp.ne.size(buffer(1:kp,:,:),1)).and.(myid.eq.0)) print *, 'process ', myid, ' z dimension unaligned on varible ', fsol%dname(n), 'with plane start ', kk

               
               
                 
               call Read3dHDF5_svariable_P(trim(fsol%fname),trim(fsol%dname(n)),kp,decomp%xsz(2),jmax, &
                                                                    kk,decomp%xst(2),decomp%xst(3), &
                                                                    buffer(1:kp,1:decomp%xsz(2),1:jmax))
                                                                    
               if(myid.eq.0) print *, 'Writing variable: ', trim(fsol%dname(n))
               call Write3dHDF5_svariable_P(trim(fsol_new%fname),trim(fsol%dname(n)),kp,imax,jmax, &
                                                                    decomp%xst(1),decomp%xst(2),decomp%xst(3), &
                                                                    buffer(1:kp,1:decomp%xsz(2),1:jmax),n-1) 
             
           enddo ! end n loop (varibles)   
             
             !ReadWrite time data
             if((myid.eq.0).and.(kk.eq.1)) call ReadHDF5_scalar(fsol,time)            
             if(myid.eq.0)call WriteHDF5_scalar(fsol_new,time)
        enddo !end kk loop (kioplane)

          
      enddo ! end nn loop (file to read)
      
    deallocate(buffer)
    call decomp_2d_finalize
    call decomp_info_finalize(decomp)
      
    endif !iFormat = 3 Using kioplane
    
    
    
    if(iFormat_flow.eq.4) then !inode
    ...
    call decomp_2d_init(kmax,imax,jmax,p_row,p_col)
    call decomp_info_init(kmax,imax,jmax,decomp) 
    !will be inverting reference to make node distrubution consistant with DNS code
    !imax-xen+1 -> xst
    
    
    !Setting up file divide in j direction
    ...
    !jres = jmax - j*jnode
     ! do n=0, jnode-1
      ...
      do nn=1, num_file !reading each file given
        !jcurrent=1 !current index for j
      
        write(unit=fnum,fmt='(I08.8)') (file_be + (nn-1)*file_skip)
        
        !do nnn=0, jnode-1 !used for spliting file in j direction jsize will be file size, jcurrent will be start

          !fsol%dimsm = (/kmax,decomp%xsz(2),jmax/) !not used currently
          ...
          if(myid.eq.0) print *, 'reading file ', trim(fsol%fname) 
              ...
              write(unit=fnum4_1,fmt='(I04.4)') imax-decomp%xen(2)+1 !istart
              write(unit=fnum4_2,fmt='(I04.4)') imax-decomp%xst(2)+1 !iend
              fsol_new%fname = 'flowdata_i'//fnum4_1//'_'//fnum4_2//'_'//fnum//'.h5'
              print *, 'writing file ... ', trim(fsol_new%fname)   
           ...
           do n=1, fsol%dnum !reading single varibles
             ...
             call Write3dHDF5_svariable_S(fsol_new,buffer,n)  
             
          enddo ! end n loop (varibles)   
             
             !ReadWrite time data
             if(myid.eq.0) then !only read on first loop per file by rank 0
               call ReadHDF5_scalar(fsol,time)             
             endif
            call MPI_Bcast(time, 8, MPI_REAL, 0, MPI_COMM_WORLD, ierr)
            
            call WriteHDF5_scalar(fsol_new,time)

              
!          call ReadHDF5_3D(fsol,buffer_flow)!(1:kmax,1:decomp%xsz(2),1:jmax),1:5)) !decompxsz(1)
!          call ReadHDF5_scalar(fsol,time) !myid if(myid.eq.0)
          
          !if(myid.eq.0)print *, buffer_flow(2,1:15,1,1)
!          fsol%offset = 0
!          write(unit=fnum4_1,fmt='(I04.4)') imax-decomp%xen(2)+1
!          write(unit=fnum4_2,fmt='(I04.4)') imax-decomp%xst(2)+1
...
!          call WriteHDF5_scalar(fsol,time)
          ...
          !jcurrent=jcurrent+jsize(nnn)
          
        !enddo !end nnn loop (j direction file split)
      enddo ! end nn loop (file to read)
    ...
    endif !end if iformat_flow =4 (inode jnode)
    
    
    if(iFormat_flow.eq.5) then !jnode
    ...
    !jmax-xen+1 -> xst
      ...
      do nn=1, num_file !reading each file given
        ...
        write(unit=fnum,fmt='(I08.8)') (file_be + (nn-1)*file_skip) 
          ...
          fsol_new%dimsf = (/decomp%xsz(1),decomp%xsz(2),decomp%xsz(3)/)
          
          if(nn.eq.1) allocate(buffer(decomp%xsz(1),decomp%xsz(2),decomp%xsz(3))) 
          
          fsol%fname = trim(flowpath_rd)//'flowdata_'//fnum//'.h5' 
          if(myid.eq.0) print *, 'reading file ', trim(fsol%fname) 
          
              !generating file names
              write(unit=fnum4_1,fmt='(I04.4)') jmax-decomp%xen(3)+1 !jstart
              write(unit=fnum4_2,fmt='(I04.4)') jmax-decomp%xst(3)+1 !jend
              fsol_new%fname = 'flowdata_j'//fnum4_1//'_'//fnum4_2//'_'//fnum//'.h5'
              print *, 'writing file ... ', trim(fsol_new%fname)   
             
           do n=1, fsol%dnum !reading and writing single varibles

             if(myid.eq.0) print *, 'Reading variable: ', trim(fsol%dname(n))
               
             call Read3dHDF5_svariable_P(fsol%fname,fsol%dname(n),decomp%xsz(1),decomp%xsz(2),decomp%xsz(3), &
                                                                  decomp%xst(1),decomp%xst(2),jmax-decomp%xen(3)+1, &
                                                                  buffer)
    ...
    endif !end if iformat_flow =5 (jnode)
...
contains
  ...
  subroutine Initial()
     ...
     call MPI_Bcast(jnode,             1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
