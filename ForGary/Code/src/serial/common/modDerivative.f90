...
!    dfdipt: deriviative evaluated at point iin
    ...
    real(8) function ddgrid_2D(iin,jin,xin,yin,vin,n)
      integer, intent(in) :: iin,jin,n
      ...
      if(size(xin).ne.size(yin).or.size(xin).ne.size(vin)) then
          print *, 'The size for input arrays (xin,yin,vin) should be the same. STOP'
          stop
      endif

      if(idim.ge.5) then
        if(iin.ge.3.and.iin.le.idim-2) then
           ddi = (vin(iin-2,jin)-8.*(vin(iin-1,jin)-vin(iin+1,jin))-vin(iin+2,jin))*c12i
        elseif(iin.eq.1) then
           ddi = -25./12.*vin(1,jin)+4.*vin(2,jin)-3.*vin(3,jin)+4./3.*vin(4,jin)-0.25*vin(5,jin)
        elseif(iin.eq.2) then
           ddi = -0.25*vin(1,jin)-5./6.*vin(2,jin)+1.5*vin(3,jin)-0.5*vin(4,jin)+1./12.*vin(5,jin)
        elseif(iin.eq.idim-1) then
           ddi = -1./12.*vin(idim-4,jin)+0.5*vin(idim-3,jin)-1.5*vin(idim-2,jin)&
                          + 5./6.*vin(idim-1,jin)+0.25*vin(idim,jin)
        elseif(iin.eq.idim) then
           ddi = 0.25*vin(idim-4,jin)-4./3.*vin(idim-3,jin)+3.*vin(idim-2,jin)&
                         - 4.*vin(idim-1,jin)+25./12.*vin(idim,jin)
        endif ! end if(iin)
     elseif(idim.eq.3.or.idim.eq.4) then
        if(iin.ge.2.and.iin.le.3) then
           ddi = -0.5*vin(iin-1,jin) + 0.5*vin(iin+1,jin)
        elseif(iin.eq.1) then
           ddi = -1.5*vin(1,jin) + 2.0*vin(2,jin) - 0.5*vin(3,jin)
        elseif(iin.eq.idim) then
           ddi = 0.5*vin(idim-2,jin) - 2.0*vin(idim-1,jin) + 1.5*vin(idim,jin)
       endif
     elseif(idim.eq.2) then
           ddi = vin(idim,jin) - vin(idim-1,jin)
    ...
    if(jdim.ge.5) then
       if(jin.ge.3.and.jin.le.jdim-2) then
           ddj = (vin(iin,jin-2)-8.*(vin(iin,jin-1)-vin(iin,jin+1))-vin(iin,jin+2))*c12i
       elseif(jin.eq.1) then
           ddj = -25./12.*vin(iin,1)+4.*vin(iin,2)-3.*vin(iin,3)+4./3.*vin(iin,4)-0.25*vin(iin,5)
       elseif(jin.eq.2) then
           ddj = -0.25*vin(iin,1)-5./6.*vin(iin,2)+1.5*vin(iin,3)-0.5*vin(iin,4)+1./12.*vin(iin,5)
       elseif(jin.eq.jdim-1) then
           ddj = -1./12.*vin(iin,jdim-4)+0.5*vin(iin,jdim-3)-1.5*vin(iin,jdim-2)&
                          + 5./6.*vin(iin,jdim-1)+0.25*vin(iin,jdim)
       elseif(jin.eq.jdim) then
           ddj = 0.25*vin(iin,jdim-4)-4./3.*vin(iin,jdim-3)+3.*vin(iin,jdim-2)&
                        - 4.*vin(iin,jdim-1)+25./12.*vin(iin,jdim)
       endif
    elseif(jdim.eq.3.or.jdim.eq.4) then
       if(jin.ge.2.and.jin.le.3) then
           ddj = -0.5*vin(iin,jin-1) + 0.5*vin(iin,jin+1)
       elseif(jin.eq.1) then
           ddj = -1.5*vin(iin,1) + 2.0*vin(iin,2) - 0.5*vin(iin,3)
       elseif(jin.eq.jdim) then
           ddj = 0.5*vin(iin,jdim-2) - 2.0*vin(iin,jdim-1) + 1.5*vin(iin,jdim)
       endif
    elseif(jdim.eq.2) then
           ddj = vin(iin,jdim) - vin(iin,jdim-1)
    elseif(jdim.eq.1) then
           ddj = 0.0
    endif

    met = Calmet_2D(Calmm_2D(iin,jin,xin,yin))
    select case(n)
      case(1) !ddx
        ddgrid_2D = ddi*met(1,1)+ddj*met(2,1)
      case(2) !ddy
        ddgrid_2D = ddi*met(1,2)+ddj*met(2,2)
      case default
        print*,'n parameter out of range'
        stop
      end select
    end function ddgrid_2D
...
!==================================================================================================
    ! Finite Difference Formulae for unequal subintervals using Lagrange's Interpolation Formulae
    ! A.K. Singh & B.S. Bhadauria. Int. Journal of Math. Analysis, Vol.3, 2009, no. 17, 815-827
    ! Three point implementation -- Diff1st_Largange3pt
    ! Inputs:
    ! iin: index of the 1D location for computing derivative
    ! xin: 1D array of spatial locations
    ! vin: 1D array of flow variables 
    ! Outputs:
    !   d(vin)/d(xin) for the location of xin(iin)
    real(8) function Diff1st_Largange3pt(iin,xin,vin)
      ...
      if(size(xin).ne.size(vin)) then
        print*,'The size for input arrays (xin,vin) MUST be the same. STOP'
        stop
      endif
      idim = size(xin)

      if(idim.eq.1) then
        Diff1st_Largange3pt = 0.
      elseif(idim.eq.2) then
        Diff1st_Largange3pt = (vin(idim)-vin(idim-1))/(xin(idim)-xin(idim-1))
      else  ! idim .ge. 3
        if(iin.eq.1) then
            h1 = xin(2)-xin(1); h2 = xin(3)-xin(2)
            Diff1st_Largange3pt = -(2.*h1+h2)*vin(iin)/(h1*(h1+h2))+(h1+h2)*vin(iin+1)/(h1*h2)-h1*vin(iin+2)/((h1+h2)*h2)
        elseif(iin.eq.idim) then
            h1 = xin(idim-1)-xin(idim-2); h2 = xin(idim)-xin(idim-1)
            Diff1st_Largange3pt = h2*vin(iin-2)/(h1*(h1+h2))-(h1+h2)*vin(iin-1)/(h1*h2)+(h1+2.*h2)*vin(iin)/((h1+h2)*h2)
        else    ! 1<iin<idim
            h1 = xin(iin)-xin(iin-1); h2 = xin(iin+1)-xin(iin)
            Diff1st_Largange3pt = -h2*vin(iin-1)/(h1*(h1+h2))-(h1-h2)*vin(iin)/(h1*h2)+h1*vin(iin+1)/((h1+h2)*h2)
        endif
      endif
    end function Diff1st_Largange3pt
